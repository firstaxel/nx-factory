...
import {
  q, createStepRunner, printSection, printSuccess, printError,
} from "../ui.js";
import {
  clerkScaffolder,
...
const PROVIDERS: Array<{ value: AuthProvider; name: string }> = [
  { value: "clerk",       name: "Clerk        — hosted UI, zero-config, generous free tier"   },
  { value: "better-auth", name: "Better Auth  — open-source, self-hosted, SQLite / Postgres"  },
  { value: "workos",      name: "WorkOS       — enterprise SSO, SCIM, MFA, hosted AuthKit UI" },
];
...
function getScaffolder(provider: AuthProvider): AuthPackageScaffolder {
  switch (provider) {
    case "clerk":       return clerkScaffolder;
    case "better-auth": return betterAuthScaffolder;
    case "workos":      return workosScaffolder;
  }
}

function providerDocs(provider: AuthProvider): string {
  switch (provider) {
    case "clerk":       return "https://clerk.com/docs";
    case "better-auth": return "https://www.better-auth.com/docs";
    case "workos":      return "https://workos.com/docs/user-management";
  }
  return "";
}
...
async function wireApps(root: string, apps: string[], pm: string): Promise<void> {
  const { default: fs } = await import("fs-extra");
  ...
  for (const appName of apps) {
    ...
    pkg.dependencies = { ...pkg.dependencies, "@workspace/auth": protocol };
...
export async function addAuthCommand(options: AddAuthOptions): Promise<void> {
  ...
  try {
    ...
    if (err instanceof MonorepoRootNotFoundError) {
      printError({
        ...
        detail:   err.message,
        recovery: [
          { label: "Initialise a new workspace:", cmd: "nx-factory init" },
      ...
      printError({
        ...
        detail:   String(err),
        recovery: [{ label: "Try running from your monorepo root:", cmd: "cd <monorepo-root>" }],
      });
    }
    process.exit(1);
  }
  ...
  if (await pathExists(authPkgDir)) {
    printError({
      ...
      recovery: [
        ...
        { label: "Then:",   cmd: "nx-factory add-auth"  },
      ],
    });
    process.exit(1);
  }
    ...
    : await inquirer.prompt(
        {
          type:    "list",
          name:    "provider",
          message: q("Auth provider", "all production-ready — pick based on your needs"),
          choices: PROVIDERS.map((p) => ({ name: p.name, value: p.value, short: p.value })),
        ...
        ...(apps.length > 0
          ? [{
              type:    "checkbox",
              name:    "selectedApps",
              message: q("Wire @workspace/auth into which apps?", "adds dep to package.json — run install after"),
              choices: apps,
              default: apps,
            }]
          : []),
  ...
  const selectedApps = (answers.selectedApps ?? defaults.selectedApps) as string[];
  ...
  if (options.dryRun) {
    ...
    await step("Write package.json, tsconfig.json, tsup.config.ts", async () => {});
    await step(`Scaffold ${scaffolder.label} ./files`,           async () => {});
    await step("Install provider npm packages",                      async () => {});
    await step(
      selectedApps.length > 0 ? `Wire into: ${selectedApps.join(", ")}` : "No apps to wire",
      async () => {},
    );
    printSuccess({
      title:    "packages/auth ready (dry run — nothing written)",
      commands: [{ cmd: `nx-factory add-auth --provider ${provider}`, comment: "run without --dry-run to apply" }],
    });
    return;
  }
  ...
  await step("Write package base files", async () => {
    await writeAuthPackageBase(authPkgDir, scaffolder, pm);
  });
  ...
  await step(
    selectedApps.length > 0
      ? `Add @workspace/auth to: ${selectedApps.join(", ")}`
      : "Skip app wiring (no apps found)",
    async () => {
      if (selectedApps.length > 0) await wireApps(root, selectedApps, pm);
    },
  );
  ...
  const nextSteps: Array<{ cmd: string; comment?: string }> = [
    ...
    { cmd: "cat packages/auth/.env.example",  comment: "copy vars to your app's .env.local"   },
    { cmd: "pnpm nx build @workspace/auth",   comment: "build the auth package"                },
  ];
  if (provider === "better-auth") {
    nextSteps.push({ cmd: "npx better-auth migrate", comment: "create DB tables" });
  }
  ...
  printSuccess({
    ...
    tips: [
      ...
      { label: "Next apps →", cmd: `import { ... } from "@workspace/auth/next"` },
      ...(provider === "better-auth"
        ? [{
            label: "Vite/Expo apps →",
            cmd: `use @workspace/auth/client and set VITE_APP_URL/NEXT_PUBLIC_APP_URL to your auth host`,
          }]
        : [{
            label: "Non-Next apps →",
            cmd: `use @workspace/auth/client (+ provider app setup), @workspace/auth/next is Next-only`,
          }]),
      { label: "Server import →", cmd: `import { ... } from "@workspace/auth/server"` },
    ],
  });
}
