...
export async function addStorybookCommand(
	...
	printSection(
		`${options.dryRun ? "[dry run] " : ""}Adding Storybook to packages/${pkgName}`,
	);
...
// ─── Story template ──────────────────────────────────────────────────────────
function buildStory(compName: string, uiPackageName: string): string {
