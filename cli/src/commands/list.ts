...
export async function listCommand(): Promise<void> {
	...
	if (!uiPkgDir) {
		printError({
			...
			recovery: [
				{ label: "", cmd: "cd <monorepo-root> && nx-factory-cli list" },
			],
		});
		process.exit(1);
		return;
			...
			for (const file of files) {
				...
				for (const comp of installed) {
					...
					if (content.includes(name)) {
						const appsForComp = importMap.get(comp) ?? [];
						if (!appsForComp.includes(app)) {
							appsForComp.push(app);
							importMap.set(comp, appsForComp);
						}
					}
				}
			}
	...
	console.log(`\n  ${c.dim("─".repeat(44))}`);
	console.log(
		`  ${c.whiteBold(uiPackageName)}  ${c.dim(`${installed.length} component${installed.length !== 1 ? "s" : ""}`)}`,
	);
	...
	if (installed.length === 0) {
		...
		console.log(
			`  ${c.dim("Run: ")}${c.purple("nx-factory-cli add-component button card")}\n`,
		);
		return;
	}
	...
	if (notExported.length > 0) {
		...
		console.log(
			`     ${c.dim("Run ")}${c.purple("nx-factory-cli doctor")}${c.dim(" to fix automatically")}`,
		);
	}
