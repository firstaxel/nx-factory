...
export async function removeComponentCommand(
	...
	if (!uiPkgDir) {
		printError({
			...
			recovery: [
				{
					label: "",
					cmd: "cd <monorepo-root> && nx-factory-cli remove-component button",
				},
			],
		});
		process.exit(1);
	...
	for (const comp of components) {
		...
		if (!exists) {
			console.log(
				`  ${c.yellow("⚠")}  ${c.dim(`${comp} — file not found, skipping`)}`,
			);
			continue;
		}
		...
		if (opts.dryRun) {
			console.log(
				`  ${c.purpleDim("│")}  ${c.dim(`[dry run] would remove: components/ui/${comp}.tsx`)}`,
			);
			removed.push(comp);
			continue;
		}
	...
	if (failed.length > 0) {
		printWarn(
			...
			"Try removing manually from components/ui/",
		);
	}
	...
	if (removed.length > 0) {
		printSuccess({
			...
			commands: opts.dryRun
				? [
						{
							cmd: "nx-factory-cli remove-component " + removed.join(" "),
							comment: "run without --dry-run to apply",
						},
					]
				: [
						{
							cmd: `pnpm nx build ${scopedPackageName(scope, cfg?.uiPackage ?? "ui")}`,
							comment: "rebuild the UI package",
						},
					],
		});
	}
