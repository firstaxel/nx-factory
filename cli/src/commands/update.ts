...
export async function updateCommand(
	...
	if (!uiPkgDir) {
		printError({
			...
			recovery: [{ label: "", cmd: "nx-factory-cli update" }],
		});
		process.exit(1);
		return;
	}
	...
	if (opts.dryRun) {
		...
		printSuccess({
			...
			commands: [
				{
					cmd: `nx-factory-cli update ${targets.join(" ")}`,
	...
	if (updated.length > 0) {
		printSuccess({
			...
			commands: [
				{
					cmd: `${pm} nx build ${scopedPackageName(scope, cfg?.uiPackage ?? "ui")}`,
					comment: "rebuild to pick up changes",
				},
			],
		});
	}
