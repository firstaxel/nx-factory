...
export function typescriptPresets(
	...
	return {
		// ── tsconfig.base.json ───────────────────────────────────────────────
		// Base workspace settings defined inside the tooling package
		"tsconfig.base.json": {
			$schema: "https://json.schemastore.org/tsconfig",
			display: "Base configuration for the workspace",
			compilerOptions: {
				target: "ES2022",
				strict: true,
...
interface AppTsConfigOptions {
	 ...
	 * Falls back to root tsconfig (tsconfig.json or tsconfig.base.json) for pre-migration workspaces.
	 * Defaults to true — all new workspaces have it.
	 */
	typescriptPkgExists?: boolean;
	rootTsConfigName?: string;
}
...
export function appTsConfig(opts: AppTsConfigOptions): object {
	...
	const rootTsConfigName = opts.rootTsConfigName ?? "tsconfig.json";
