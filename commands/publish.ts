...
import { c, q, createStepRunner, printSection, printSuccess, printError, printWarn } from "../ui.js";
...
export async function publishCommand(options: PublishOptions): Promise<void> {
  ...
  printSection(`${options.dryRun ? "[dry run] " : ""}Publishing ${pkgName}@${nextVersion}`);
  ...
  // ─── Step 3: Build ────────────────────────────────────────────────────────
  await step("Build package (tsup)", async () => {
  ...
  printSuccess({
    ...
    commands: [
      { cmd: `npm install ${pkgName}@${nextVersion}`, comment: "install in a new project" },
      { cmd: `npm dist-tag ls ${pkgName}`,           comment: "verify dist-tags" },
    ],
    tips: options.dryRun
      ? [{ label: "This was a dry run — nothing was published.", cmd: "Remove --dry-run to publish for real" }]
      : [{ label: "View on npm:", cmd: `https://www.npmjs.com/package/${pkgName}` }],
  });
}
